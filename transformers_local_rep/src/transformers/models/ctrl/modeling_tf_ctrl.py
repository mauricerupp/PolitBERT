...
class TFCTRLModel(TFCTRLPreTrainedModel):
    ...
    # Copied from transformers_local.models.gpt2.modeling_tf_gpt2.TFGPT2Model.serving_output
...
class TFCTRLLMHeadModel(TFCTRLPreTrainedModel, TFCausalLanguageModelingLoss):
    ...
    # Copied from transformers_local.models.gpt2.modeling_tf_gpt2.TFGPT2LMHeadModel.serving_output
    def serving_output(self, output):
        pkv = tf.convert_to_tensor(output.past_key_values) if self.config.use_cache else None
        hs = tf.convert_to_tensor(output.hidden_states) if self.config.output_hidden_states else None
        attns = tf.convert_to_tensor(output.attentions) if self.config.output_attentions else None
...
@add_start_docstrings(
    ...
    :class:`~transformers_local.TFCTRLForSequenceClassification` uses the last token in order to do the classification, as
