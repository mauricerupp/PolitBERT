...
def check_dummies(overwrite=False):
    ...
    for backend in dummy_files.keys():
        if dummy_files[backend] != actual_dummies[backend]:
            if overwrite:
                print(
                    f"Updating transformers_local.utils.dummy_{short_names.get(backend, backend)}_objects.py as the main "
                ...
                raise ValueError(
                    ...
                    f"transformers_local.utils.dummy_{short_names.get(backend, backend)}_objects.py. Run `make fix-copies` "
                    "to fix this."
                )
