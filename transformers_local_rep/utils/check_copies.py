...
TRANSFORMERS_PATH = "src/transformers_local"
...
def find_code_in_transformers(object_name):
    ...
    if i >= len(parts):
        raise ValueError(
            f"`object_name` should begin with the name of a module of transformers_local but got {object_name}."
        )
...
_re_copy_warning = re.compile(r"^(\s*)#\s*Copied from\s+transformers_local\.(\S+\.\S+)\s*($|\S.*$)")
