...
PATH_TO_TRANSFORMERS = "src/transformers_local"
PATH_TO_TESTS = "tests"
...
# This is to make sure the transformers_local module imported is the one in the repo.
...
def get_model_modules():
    """ Get the model modules inside the transformers_local library. """
    _ignore_modules = [
        "modeling_auto",
...
def find_all_documented_objects():
    ...
    for doc_file in Path(PATH_TO_DOC).glob("**/*.rst"):
        with open(doc_file, "r", encoding="utf-8", newline="\n") as f:
            content = f.read()
        raw_doc_objs = re.findall(r"(?:autoclass|autofunction):: transformers_local.(\S+)\s+", content)
